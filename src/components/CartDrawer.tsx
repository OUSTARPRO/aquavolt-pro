...
export default function CartDrawer() {
  ...
  const handleWhatsApp = () => {
    ...
    const msg = [
      "🍕 *Nuovo ordine – Pizzeria Siciliana Bar Lipari*",
      "",
      ...lines,
      "",
      `*Totale: ${total.toFixed(2)}€*`,
      "",
      "_(Consegna o da asporto?)_",
    ].join("\n");
    ...
    <>
      ...
      >
        ...
        <div className="flex items-center justify-between p-5 border-b border-white/10">
          ...
          <button onClick={toggleCart} className="p-2 rounded-full hover:bg-white/10 transition-colors">
        ...
        <div className="flex-1 overflow-y-auto p-5 space-y-3">
          {items.length === 0 ? (
            <div className="flex flex-col items-center justify-center h-full text-center gap-4">
              ...
              <p className="text-white/60 font-medium">Il carrello è vuoto</p>
              <p className="text-white/40 text-sm">Aggiungi le tue pizze preferite!</p>
              <button
                onClick={toggleCart}
                className="mt-2 px-6 py-2.5 bg-fire rounded-full text-white font-semibold text-sm glow-fire-sm"
              >
                Vedi il menu
              </button>
            </div>
            ...
            items.map((item) => (
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  ...
                  {item.size && <p className="text-white/40 text-xs">{item.size}</p>}
                  <p className="text-orange-400 font-bold text-sm mt-0.5">
                    {(item.price * item.quantity).toFixed(2)}€
                  </p>
                </div>
                <div className="flex items-center gap-2">
                  <button onClick={() => updateQuantity(item.id, item.quantity - 1)} className="w-7 h-7 rounded-full bg-white/10 hover:bg-red-600/60 flex items-center justify-center transition-colors">
                    <Minus className="w-3 h-3 text-white" />
                  </button>
                  <span className="text-white font-bold text-sm w-4 text-center">{item.quantity}</span>
                  <button onClick={() => updateQuantity(item.id, item.quantity + 1)} className="w-7 h-7 rounded-full bg-white/10 hover:bg-green-600/60 flex items-center justify-center transition-colors">
                    <Plus className="w-3 h-3 text-white" />
                  </button>
                </div>
                <button onClick={() => removeItem(item.id)} className="p-1.5 text-white/30 hover:text-red-400 transition-colors">
        ...
        {items.length > 0 && (
          <div className="border-t border-white/10 p-5 space-y-4">
            ...
            <button onClick={clearCart} className="w-full text-white/40 hover:text-white/70 text-sm transition-colors py-1">
              Svuota carrello
            </button>
          </div>
        )}
